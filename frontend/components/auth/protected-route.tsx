...
export function ProtectedRoute({ children, roles }: { children: React.ReactNode; roles?: UserRole[] }) {
  ...
  useEffect(() => { if (loading) return; if (!user) router.replace('/login'); if (user && roles?.length && !roles.includes(user.role)) { const home = user.role === 'coach' ? '/coach/home' : user.role === 'super_admin' ? '/admin' : '/client/home'; router.replace(home); } }, [loading, user, roles, router]);
