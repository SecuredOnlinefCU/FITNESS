      ...
      <CardContent className="p-5">
        ...
        <div className="space-y-3">
          {(warnings.data?.items || []).length ? warnings.data!.items.map((item) => (
            <div key={item.id} className="rounded-2xl border border-border p-4"><div className="flex gap-3"><AlertTriangle className="h-5 w-5 text-primary" /><div><p className="font-black">{item.title}</p><p className="text-sm text-slate-500">{item.body}</p><p className="mt-1 text-xs font-bold text-slate-400">{item.warningType} • {item.severity}</p></div></div></div>
          )) : <p className="text-sm text-slate-500">No active workout warnings.</p>}
        </div>
      </CardContent>
    </Card>
  );
