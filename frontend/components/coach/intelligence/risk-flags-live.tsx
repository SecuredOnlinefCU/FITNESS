...
export function RiskFlagsLive({ clientUserId }: { clientUserId?: string }) {
  ...
  if (!flags.data?.items?.length) return <EmptyState title="No open risk flags" description="Client risk signals will appear here when LevelFit detects something that needs coach attention." />;
