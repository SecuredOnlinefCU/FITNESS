      ...
      <CardContent className="flex h-[calc(100vh-220px)] min-h-[520px] flex-col p-4">
        <div className="mb-4 flex items-start justify-between gap-4 border-b border-border pb-4">
          <div>
            ...
            <p className="text-sm text-slate-500">WebSocket-first messaging with HTTP fallback and delivery states.</p>
          </div>
          ...
          {thread.messages.length ? thread.messages.map((message: any) => {
            ...
            return (
              <div key={message.clientMessageId || message.id} className={cn('flex', mine ? 'justify-end' : 'justify-start')}>
                <div className={cn('max-w-[80%] rounded-2xl px-4 py-3 text-sm shadow-sm', mine ? 'bg-primary text-primaryForeground' : 'border border-border bg-white', message.deliveryStatus === 'failed' ? 'border-red-300 bg-red-50 text-red-700' : '')}>
                  <p>{message.bodyText || `[${message.messageType}]`}</p>
                  <div className={cn('mt-1 flex justify-end', mine ? 'text-white/75' : 'text-slate-400')}>
